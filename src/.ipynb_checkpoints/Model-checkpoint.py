...
class Model: 
	...
	def __init__(self, charList, decoderType=DecoderType.BestPath, mustRestore=False, dump=False):
		"inicia el mdelo: añade CNN, RNN and CTC y le inicializa TF"
		self.dump = dump
		...
		# configura la optimizacion para train
	...
	def setupCNN(self):
		"crea las capas  y devuelve la salida"
		cnnIn4d = tf.expand_dims(input=self.inputImgs, axis=3)

		#lista de lo que va en las capas
	...
	def setupRNN(self):
		"crea las capas rnn y las configura"
	...
	def setupCTC(self):
		...
		# tenemos el texto real como tensor
		self.gtTexts = tf.SparseTensor(tf.placeholder(tf.int64, shape=[None, 2]) , tf.placeholder(tf.int32, [None]), tf.placeholder(tf.int64, [2]))

		# calcula perdida del tole
		...
		if self.decoderType == DecoderType.BestPath:
			...
			# Prepara informacion sobre el lenguaje
	...
	def setupTF(self):
		...
		# si el modelo se reinicia, hay captura
		if self.mustRestore and not latestSnapshot:
			raise Exception('No saved model found in: ' + modelDir)

		# carga modelo si hay disponibilidad
		if latestSnapshot:
	...
	def decoderOutputToText(self, ctcOutput, batchSize):
		"extrae text desde la salida de CTC decoder"
		...
		# Decodificadores TF: las cadenas de etiquetas están contenidas en un tensor disperso
		else:
			#retorna el primer elemento tensor
	...
	def dumpNNOutput(self, rnnOutput):
		...
		# iitera sobre elementos del lote y calcula la salida
	...
	def inferBatch(self, batch, calcProbability=False, probabilityOfGT=False):
		"alimentar un lote en el NN para reconocer los textos"
